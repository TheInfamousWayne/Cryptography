...
class GUI(Frame):
    ...
    def update_lists(self, type, id, price, vol):
        if index < len(self.buy_cid_file):
            self.buy_cid_msg.append(self.buy_cid_file[index])
            self.buy_price_msg.append(self.buy_price_file[index])
            self.buy_vol_msg.append(self.buy_vol_file[index])
            #############
            self.sell_cid_msg.append(self.sell_cid_file[index])
            self.sell_price_msg.append(self.sell_price_file[index])
            self.sell_vol_msg.append(self.sell_vol_file[index])
            ###############################################
            ...
            ################################################
            self.root.after(UPDATE_RATE, self.update_lists, index+1)
